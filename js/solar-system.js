...
function animate() {
  ...
  planetObjects.forEach(({ planet, orbit, data }) => {
    ...
    const semiMinor =
      data.distance * Math.sqrt(1 - data.eccentricity * data.eccentricity);
    const x = Math.cos(angle) * semiMajor;
    const z = Math.sin(angle) * semiMinor;
